...
export function JourneyCard({
  ...
  return (
    <div
      className={`group surface-panel border p-5 transition-[border-color,box-shadow,transform] duration-300 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-[var(--shadow-elevated)] sm:p-6 ${
        isActive
        ...
        <div className="flex items-center gap-2">
          {onDelete ? (
            <button
              ...
              className="flex size-4 shrink-0 items-center justify-center text-muted-foreground transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 hover:text-destructive"
              onClick={(event) => {
          ...
          <dl className="mt-5 grid gap-3 border-t border-border/60 pt-4 sm:grid-cols-2">
            <div className="min-w-0">
              ...
              <dd className="mt-1 truncate text-sm text-foreground">
                {nextMove?.phase ??
                  (isComplete ? "Journey complete" : "Not started")}
              </dd>
            </div>
