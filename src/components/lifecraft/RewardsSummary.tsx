...
export function RewardsSummary({ className = "" }: { className?: string }) {
  ...
  return (
    <div
      className={`grid gap-3 sm:grid-cols-[minmax(0,1.35fr)_minmax(190px,0.65fr)] ${className}`}
    >
      <button
        type="button"
        aria-expanded={openPanel === "xp"}
        onClick={() =>
          setOpenPanel((prev) => (prev === "xp" ? null : "xp"))
        }
        ...
        <div className="flex items-center justify-between gap-3">
          <span className="inline-flex items-center gap-2.5 text-sm text-foreground">
            ...
            <span>
              ...
              <span className="mt-0.5 block font-display text-sm font-semibold">
                Level {level}
              </span>
            </span>
          </span>
      ...
      <div className="relative">
        <button
          ...
          onClick={() =>
            setOpenPanel((prev) => (prev === "gems" ? null : "gems"))
          }
          className="group w-full rounded-2xl border border-border bg-surface-2/60 px-5 py-4 text-left transition-[border-color,box-shadow,transform] duration-300 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-[var(--shadow-elevated)]"
        >
          <div className="flex items-center justify-between gap-3">
            <span className="inline-flex items-center gap-2.5 text-sm text-foreground">
              <span className="grid size-8 place-items-center rounded-lg bg-accent-soft">
        ...
        {openPanel === "gems" ? (
          <>
            <div className="absolute left-0 right-0 top-full z-30 mt-2 rounded-xl border border-border bg-surface p-4 shadow-[var(--shadow-elevated)] sm:hidden">
              ...
              {gemHistory.length === 0 ? (
                <p className="mt-2 text-xs text-muted-foreground">
                  No gem rewards yet — complete a step to earn some.
                </p>
              ) : (
            ...
            <div className="mt-3 hidden space-y-1.5 border-t border-border pt-3 text-xs text-muted-foreground sm:block">
