      ...
      {journeys.length > 0 ? (
              ...
              <JourneyCard
                ...
                onDelete={() => {
                  const confirmed = window.confirm(
                    `Delete "${journey.title}"?\n\nThis journey and its progress will be permanently removed.`,
                  );
          ...
          <section className="surface-panel mt-6 flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div>
              <h2 className="text-base font-semibold">
                Room for one more intention
              </h2>

              <p className="mt-1 max-w-xl text-sm text-muted-foreground">
                Describe a goal in plain words and LIFECRAFT will shape the
                journey around it.
              </p>
            </div>
        ...
        <section className="surface-panel mt-8 p-6 sm:p-8">
          ...
          <p className="mt-2 max-w-xl text-sm leading-relaxed text-muted-foreground">
            Describe a goal in plain words and LIFECRAFT will shape it into a
            clear path with a next move.
          </p>
